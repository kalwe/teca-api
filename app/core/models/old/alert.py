...
class Alert(BaseEntity):
    ...
    alert_type = fields.CharField(
        max_length=100, description="Type of alert (e.g., Birthday, Contract Expiry)")
    alert_message = fields.TextField(description="The content of the alert")
    recipient_email = fields.CharField(
        max_length=255, description="Email address of the recipient")
    ...
    def trigger_event(self, event_emitter: 'EventEmitter') -> None:
        ...
        alert_event = AlertEvent(
            self.alert_type, self.alert_message, self.recipient_email)
    ...
    def emit(self, event: AlertEvent) -> None:
        ...
        for observer in self._observers:
            observer.send_notification(
                event.alert_message, event.recipient_email)
