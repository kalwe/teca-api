...
from app.api.schemas.reminder_schema import ReminderDeletedSchema, ReminderInputSchema, ReminderOutputSchema
from app.core.repositories.reminder.reminder_create_repository import ReminderCreateRepository
from app.core.repositories.reminder.reminder_delete_repository import ReminderDeleteRepository
...
class ReminderController:
    ...
    async def get_reminder(id: int):
        ...
        """
        service = ReminderGetService(ReminderGetRepository())
    ...
    async def get_all_reminders():
        ...
        """
        service = ReminderGetService(ReminderGetRepository())
        reminders = service.get_all()
        return reminders, HTTPStatus.OK

    @staticmethod
    @validate_request(ReminderInputSchema)
    @validate_response(ReminderOutputSchema)
    async def update_reminder(id: int, data: ReminderInputSchema):
        repository = ReminderUpdateRepository()
        service = ReminderUpdateService(repository)
        reminder = service.update(id, data)
        return reminder, HTTPStatus.OK
